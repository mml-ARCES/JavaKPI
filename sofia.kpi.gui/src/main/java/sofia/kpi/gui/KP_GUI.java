...
import sofia_kp.ArcesServiceRegistry;
...
public class KP_GUI  extends Panel implements ActionListener, iKPIC_subscribeHandler2 
{
	...
	JLabel label_sub;
	private JLabel label_polPM;
	private JLabel label_polA;
	private JLabel label_pol;
	...
	public KP_GUI()
	{
		...
		bLastWill = new JButton("Register LastWill");
		bRULE = new JButton("Persistent update");

		bSPARQLquery.addActionListener(this);
		bLastWill.addActionListener(this);
	...
	else if( c == bSPARQLquery)
	{
		...
		taMemo.setText("");
		taMemo.append("\nLastWill Subscription!"); 
		
		ret = kp.lastwill_sparql(tfSQ.getText());
		
		history.append("Sparql lastwill s = " + tfSQ.getText() + "\n");

		taMemo.append("Sent: " + kp.xmlTools.lastWillSPARQL(tfSQ.getText()) + "\n");
		taMemo.append("SIB MESSAGE:\n"+ret+"\nKP-CORE MESSAGE:"+kp.getErrMess()+"("+kp.getErrID()+")\n");

		taMemo.append("LastWill confirmed:"+(this.xmlTools.isResponseConfirmed(ret)?"YES":"NO")+"\n");
		taMemo.append("\n*** SSAP message status:"+ret.Status +"\n");
	...
	{ taMemo.setText("");
	ret = kp.subscribeRDFReconnection(  
			tfSN.getText().equals("*") ? "http://www.nokia.com/NRC/M3/sib#any" : tfSN.getText()
					,tfPN.getText().equals("*") ? "http://www.nokia.com/NRC/M3/sib#any" : tfPN.getText() 
							,tfON.getText().equals("*") ? "http://www.nokia.com/NRC/M3/sib#any" : tfON.getText()
