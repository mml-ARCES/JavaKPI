...
public class SSAP_sparql_response {
	...
	public void initFromResults(Vector<Vector<String[]>> sparql_results){
		if(sparql_results == null){
			throw new IllegalArgumentException("sparql_results is null");
		}
		
		sparql_response_results = sparql_results;
		has_results = !sparql_response_results.isEmpty();
		
		has_booleans = false;
		
		link_hrefs = new Vector<>();
		var_names = new Vector<>();
		for(Vector<String[]> v : sparql_results){
			for(String[] data : v){
				var_names.add(data[VARNAME]);
			}
		}
