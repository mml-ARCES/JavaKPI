...
import sofia_kp.subscriptions.AutoStateFullSubscription;
import sofia_kp.subscriptions.AutoSubscription;
import sofia_kp.subscriptions.SubscriptionQOS;
...
public class KPICore implements iKPIC, iKPIC_subscribeHandler2,  iKPIC_subscribeHandler
{
	...
	/**
	 * Register a lastwill SPARQL update  in the SIBs supporting it
	 * @param sparql_update a SPARQL update query
	 * @return the SIB answer
	 * @Todo This method currently supports sparql update for SIB 0.9 only with protocol version = 0 (default) 
	 */
	...
	public SIBResponse lastwill_sparql(String sparql_update )
	{
		deb_print("KpCore:lastwill_method");
		int ret=0;
		
		String msg = this.xmlTools.lastWillSPARQL(sparql_update);
		...
		if(ret!=this.ERR_Conected )
		{
			...
			err_print("ERROR:lastwill:connection error:" + this.getErrMess()); 
			return null;
		}
		...
		{
			this.KP_ERROR_ID=ret;
			err_print("ERROR:lastwill:send error:"+this.getErrMess()); 
			return null;
		}
		...
		try
		{
			while ((charRead = in.read(buffer, 0, buffer.length)) != (-1)) 
			{
				...
				if(msg.contains("<SSAP_message>") && msg.contains(  "</SSAP_message>") )
				{	//The first message is not an event but just the confirmation message!!!
					if(this.xmlTools.getParameterElement(msg,"name", "status").getValue().equals("m3:Success"))
					{
						deb_print("LastWill demon thread starts");
						kpSocket.setKeepAlive(true);

						KeepMeAlive activeThread = new KeepMeAlive(kpSocket);
						activeThread.start();
	...
	public SIBResponse subscribeRDF(String s,String p,String o,String o_type, iKPIC_subscribeHandler2 handler)
	{
		...
		return subscribe( this.xmlTools.subscribeRDF(s, p, o, o_type ) , handler,SubscriptionQOS.NOSERVICE);
	}//public String subscribeRDF(String s,String p,String o,String o_type)
	public SIBResponse subscribeRDFReconnection(String s,String p,String o,String o_type, iKPIC_subscribeHandler2 handler)
	{
		deb_print("\n[SUBSCRIBE RDF Reconnection]___________________________________"); 
		//		System.out.println("DEBUG: " + this.xmlTools.subscribeRDF(s, p, o, o_type));

	

		return subscribe( this.xmlTools.subscribeRDF(s, p, o, o_type ) , handler,SubscriptionQOS.RECONNECTION);
	}
	public SIBResponse subscribeRDFContextRecovery(String s,String p,String o,String o_type, iKPIC_subscribeHandler2 handler)
	{
		deb_print("\n[SUBSCRIBE RDF Reconnection]___________________________________"); 
		//		System.out.println("DEBUG: " + this.xmlTools.subscribeRDF(s, p, o, o_type));
		...
		return subscribe( this.xmlTools.subscribeRDF(s, p, o, o_type ) , handler,SubscriptionQOS.CONTEXTRECOVERY);
	}
	public SIBResponse subscribeRDF(Vector<Vector<String> > triples, iKPIC_subscribeHandler2 handler)
	{
		deb_print("\n[SUBSCRIBE RDF]___________________________________"); 
		//System.out.println("DEBUG: " + this.xmlTools.subscribeRDF(s, p, o, o_type));
		return subscribe( this.xmlTools.subscribeRDF(triples), handler,SubscriptionQOS.NOSERVICE);
	}//public String subscribeRDF(String s,String p,String o,String o_type)
	
	public SIBResponse subscribeRDFReconnection(Vector<Vector<String> > triples, iKPIC_subscribeHandler2 handler)
	{
		deb_print("\n[SUBSCRIBE RDF WITH RECONNECTION]___________________________________"); 
		//System.out.println("DEBUG: " + this.xmlTools.subscribeRDF(s, p, o, o_type));
		return subscribe( this.xmlTools.subscribeRDF(triples), handler,SubscriptionQOS.RECONNECTION);
	}
	
	public SIBResponse subscribeRDFContextRecovery(Vector<Vector<String> > triples, iKPIC_subscribeHandler2 handler)
	{
		deb_print("\n[SUBSCRIBE RDF WITH RECONNECTION]___________________________________"); 
		//System.out.println("DEBUG: " + this.xmlTools.subscribeRDF(s, p, o, o_type));
		return subscribe( this.xmlTools.subscribeRDF(triples), handler,SubscriptionQOS.CONTEXTRECOVERY);
	}
	...
	public SIBResponse subscribeSPARQL(String query, iKPIC_subscribeHandler2 handler)
	{
		...
		return subscribe( this.xmlTools.subscribeSPARQL(query), handler,SubscriptionQOS.NOSERVICE );

	}//public String subscribeSPARQL(String query)
	
	public SIBResponse subscribeSPARQLReconnection(String query, iKPIC_subscribeHandler2 handler)
	{
		deb_print("\n[SUBSCRIBE SPARQL with reconnection]___________________________________"); 

		return subscribe( this.xmlTools.subscribeSPARQL(query), handler,SubscriptionQOS.RECONNECTION );

	}
	
	public SIBResponse subscribeSPARQLContextRecovery(String query, iKPIC_subscribeHandler2 handler)
	{
		deb_print("\n[SUBSCRIBE SPARQL with reconnection]___________________________________"); 

		return subscribe( this.xmlTools.subscribeSPARQL(query), handler,SubscriptionQOS.CONTEXTRECOVERY);
	}
	/* * * * * * * * * * * * * *\
	 ...
	 * @param auto_reconnection true if you want to automatic reconnect 
	 * 
	 * @return null in case of error otherwise the SIB answer to the 
	 * subscription request
	 */
	private SIBResponse subscribe(String query, iKPIC_subscribeHandler2 handler,SubscriptionQOS qos)
	{
			...
			while ((charRead = in.read(buffer, 0, buffer.length)) != (-1)) 
			{
				...
				{	//The first message is not an event but just the confirmation message!!!
					if(this.xmlTools.isSubscriptionConfirmed(msg))
					{
						...
						switch (qos) {
						case NOSERVICE:
							s = new Subscription(kpSocket, handler);
							System.out.println ("Subscription: going to start thread");
							break;
						case RECONNECTION:
							s = new AutoSubscription(kpSocket, handler, new InetSocketAddress(HOST, PORT), query ,this);
							break;
						case CONTEXTRECOVERY:
							s = new AutoStateFullSubscription(kpSocket, handler, new InetSocketAddress(HOST, PORT), query ,this,new SIBResponse(msg));
							System.out.println ("Subscription: going to start thread with reconnection");
							break;
						default:
							s = new Subscription(kpSocket, handler);
							System.out.println ("Subscription: going to start thread");
